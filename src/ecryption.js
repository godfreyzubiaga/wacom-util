// Warning: this implementation uses hardcoded encryption keys, those should be properly generated in a real application
import "big-integer";
...
function arrayToHex(v) {
  var s = "";
  for (var i = 0; i < v.length; ++i) s = s + toHex2(v[i]);
  return s;
}
...
function padLeft(str, len, pad) {
  ...
  str = str.toString();
  if (len > str.length) str = Array(len + 1 - str.length).join(pad) + str;
  return str;
}
...
export function encryptionHandler() {
  ...
  encryptionHandler.setDH = function (dhPrime, dhBase) {
    var p = dhPrime; //js.toArray(dhPrime);
    var g = dhBase; // js.toArray(dhBase);
...
export function encryptionHandler2() {
  ...
  encryptionHandler2.getParameters = function () {
    return [2, 0, 2];
  };
  ...
  encryptionHandler2.generatePublicKey = function () {
    ...
    if (calculatePrimes) {
      ...
      while (1) {
        ...
        if (!equalsInt(mod(bigint_p, this.bigint_e), 1))
      ...
      while (1) {
        ...
        while (1) {
          ...
          if (bigint_p != bigint_q) {
            if (!equalsInt(mod(bigint_q, this.bigint_e), 1))
              //the prime must not be congruent to 1 modulo e
              break;
          }
        }
